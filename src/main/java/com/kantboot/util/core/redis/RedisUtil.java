package com.kantboot.util.core.redis;
...
public class RedisUtil {
    ...
    public RedisUtil setStringRedisTemplate(StringRedisTemplate stringRedisTemplate) {
    ...
    public Boolean lock(String key, long timeout, TimeUnit unit) {
        ...
        stringRedisTemplate.opsForValue().set(key, "1", timeout, unit);
        return false;
...
//        return stringRedisTemplate.opsForValue().setIfAbsent(key, "1");
    ...
    /**
     * 设置过期时间
     * 与expireAt不同的是，expireAt是设置某个时间点过期，而expire是设置多少秒后过期
     * @param key    键
     * @param timeout 时间
     * @param unit 时间单位
    ...
    /**
     * 设置过期时间
     * 与expire不同的是，expire是设置多少秒后过期，而expireAt是设置某个时间点过期
     * @param key 键
     * @param date 时间
